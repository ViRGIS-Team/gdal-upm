...
public class Transformer : global::System.IDisposable {
  ...
  static private global::System.IntPtr SwigConstructTransformer(Dataset src, Dataset dst, string[] options) {
    using (var tempoptions = new GdalPINVOKE.StringListMarshal(options)) { 
    return GdalPINVOKE.new_Transformer(Dataset.getCPtr(src), Dataset.getCPtr(dst), tempoptions._ar);
    }
  }

  public Transformer(Dataset src, Dataset dst, string[] options) : this(Transformer.SwigConstructTransformer(src, dst, options), true, null) {
  ...
  public int TransformGeolocations(Band xBand, Band yBand, Band zBand, Gdal.GDALProgressFuncDelegate callback, string callback_data, string[] options) {
    ...
    using (var tempoptions = new GdalPINVOKE.StringListMarshal(options)) { 
    try {
      int ret = GdalPINVOKE.Transformer_TransformGeolocations(swigCPtr, Band.getCPtr(xBand), Band.getCPtr(yBand), Band.getCPtr(zBand), callback, tempcallback_data, tempoptions._ar);
      if (GdalPINVOKE.SWIGPendingException.Pending) throw GdalPINVOKE.SWIGPendingException.Retrieve();
      return ret;
    } finally {
